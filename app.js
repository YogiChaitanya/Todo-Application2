...
app.get('/todos/', async (request, response) => {
  ...
  const todoArray4 = await db.all(getTodoQueryS4)
  response.send(
    todoArray4.map(eachQuery => convertDBObjectToResponseObject(eachQuery)),
  )
})
...
app.put('/todos/:todoId/', async (request, response) => {
  ...
  // scenario 2
  const updateTodoS2 = `UPDATE todo SET priority='${priority}' WHERE id=${todoId};`
  await db.run(updateTodoS2)
  response.send('Priority Updated')

  // scenario 3
  const updateTodoS3 = `UPDATE todo SET todo='${todo}' WHERE id=${todoId};`
